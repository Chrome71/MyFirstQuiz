...
def show_start_screen():
    ...
    options = ["5", "15", "25"]
    option_surfaces = [option_font.render(f'Press {i+1} to select {opt} questions', True, WHITE) for i, opt in enumerate(options)]
    for i, opt_surface in enumerate(option_surfaces):
        screen.blit(opt_surface, (WIDTH // 2 - opt_surface.get_width() // 2, HEIGHT // 2 + 100 + i * 50))
    pygame.display.flip()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in [pygame.K_1, pygame.K_2, pygame.K_3]:
                    selected = int(options[event.key - pygame.K_1])
    return selected
...
def show_end_screen(score, won):
    ...
    screen.blit(end_surface, (WIDTH // 2 - end_surface.get_width() // 2, HEIGHT // 2 - end_surface.get_height() // 2 - 50))
    score_surface = font.render(f'Score: {score}', True, WHITE)
    screen.blit(score_surface, (WIDTH // 2 - score_surface.get_width() // 2, HEIGHT // 2 - score_surface.get_height() // 2 + 50))
    pygame.display.flip()
...
def main_game(question_count):
    ...
    while running:
        ...
        screen.blit(level_surface, (WIDTH - 120, 10))
        ...
        screen.blit(input_surface, (input_box.x + 5, input_box.y + 2))
